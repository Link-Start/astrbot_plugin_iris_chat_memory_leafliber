...
async def login():
    ...
    if not expected_key:
        response = jsonify({"success": True, "message": "无需认证"})
        return response

    if verify_access_key(provided_key):
        response = jsonify({"success": True, "message": "认证成功"})
        response.set_cookie(
            "iris_access_key",
            provided_key,
            max_age=30 * 24 * 3600,
        ...
        return jsonify(
            {"success": False, "error": "访问密钥错误", "code": "AUTH_FAILED"}
        ), 401
...
async def auth_status():
    ...
    if not expected_key:
        return jsonify({"require_auth": False, "authenticated": True})
    ...
    return jsonify({"require_auth": True, "authenticated": authenticated})

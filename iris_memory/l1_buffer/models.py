...
class MessageQueue:
    ...
    Note:
        本类非线程安全。在 asyncio 单线程事件循环中，同步方法（add_message、
        remove_oldest 等）在同一个 await 点之间不会被中断，因此是安全的。
        但如果跨 await 点操作同一队列（如总结过程中），需由调用方加锁保护。
    ...
    def remove_oldest(self) -> Optional[ContextMessage]:
        ...
        self.total_tokens = max(0, self.total_tokens - message.token_count)
        return message
    ...
    def split_for_summary(
        self, retain_count: int, max_retain_tokens: Optional[int] = None

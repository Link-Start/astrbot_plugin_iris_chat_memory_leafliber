...
class GraphEnhancer:
        ...
        try:
            ...
            if user_id:
                user_node_ids = await self._search_nodes_by_user(
                    user_id=user_id, adapter=adapter, group_id=group_id
                )
            ...
            if keywords:
                keyword_node_ids = await self._search_nodes_by_keywords(
                    keywords=keywords, adapter=adapter, group_id=group_id
                )
            ...
            nodes, edges = await retriever.retrieve_with_expansion(
                memory_node_ids=list(all_node_ids), group_id=group_id
            )
    ...
    def _extract_keywords(
        self, query: Optional[str], memories: List[MemorySearchResult]
    ) -> Set[str]:
    ...
    async def _search_nodes_by_user(
        self, user_id: str, adapter: L3KGAdapter, group_id: Optional[str] = None
    ) -> Set[str]:
        """搜索与用户ID相关的图谱节点
        ...
        try:
            ...
            result1 = adapter._conn.execute(
                query1, {"user_id": user_id, "group_id": group_id}
            )
            ...
            if matched_ids:
                ...
                result2 = adapter._conn.execute(
                    query2, {"user_node_id": user_node_id, "group_id": group_id}
                )
            ...
            result3 = adapter._conn.execute(
                query3, {"user_id": user_id, "group_id": group_id}
            )
    ...
    async def _search_nodes_by_keywords(
        self, keywords: Set[str], adapter: L3KGAdapter, group_id: Optional[str] = None
    ) -> Set[str]:
        """在图谱中搜索匹配关键词的节点
        ...
        try:
            ...
            for keyword in keywords:
                ...
                result = adapter._conn.execute(
                    query, {"keyword": keyword, "group_id": group_id}
                )

...
class EnhancedMemoryRetriever:
    ...
    async def retrieve_with_enhancement(
        ...
        if enable_graph_flag:
            memories, graph_context = await self._graph_enhancer.enhance(
                memories=memories, group_id=group_id, query=query
            )
        ...
        memories, actual_tokens = self._budget_controller.trim_memories(
            memories=memories, max_tokens=budget
        )

        logger.debug(
            f"Token 预算裁剪后：{len(memories)} 条记忆，{actual_tokens} tokens"
        )
        ...
        if enable_rerank_flag:
            ...
            if reranker:
                memories = await reranker.rerank(memories=memories, query=query)
    ...
    def _format_context(
        self, memories: List[MemorySearchResult], graph_context: str
    ) -> str:
        """格式化记忆和图谱为上下文文本

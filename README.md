...
| 资源 | 最低要求 | 推荐配置 |
|------|----------|----------|
| **Python** | 3.12.4+ | 最新 3.12/3.13 |
| **AstrBot** | 带插件 WebAPI 鉴权的版本 | 最新版 |
| **内存** | 2 GB 可用 | 4 GB+ 可用 |
| **磁盘** | 500 MB | 2 GB+（随记忆量增长） |
| **CPU** | 2 核 | 4 核+ |

> **Python ≥ 3.12.4**：图片 SSRF 防护依赖 `ipaddress.is_global` 对 CGNAT（100.64.0.0/10）等保留网段的正确判定，旧版本会将部分保留地址误判为全局地址。
>
> **AstrBot 版本**：Web 管理界面的鉴权由 AstrBot Dashboard 提供，请勿将 Dashboard 弱口令暴露公网；旧版宿主若缺少插件 WebAPI 鉴权，不应启用本插件的 Web 界面。
>
> **h11 ≥ 0.16.0**：httpx 的传递依赖，低于该版本存在 CRLF 请求走私漏洞（CVE-2025-43859），部署环境请确保已升级。

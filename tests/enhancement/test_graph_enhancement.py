...
def create_memory_result(content: str, node_id: str = None) -> MemorySearchResult:
    ...
    entry = MemoryEntry(id=f"mem_{hash(content)}", content=content, metadata=metadata)
...
class TestGraphEnhancer:
    ...
    async def test_enhance_disabled(self, mock_component_manager):
        """测试图增强未启用"""
        with patch(
            "iris_memory.enhancement.graph_enhancement.get_config"
        ) as mock_get_config:
    ...
    async def test_enhance_with_valid_node_ids(self, mock_component_manager):
        with (
            ...
            patch(
                "iris_memory.enhancement.graph_enhancement.GraphRetriever"
            ...
            mock_retriever.retrieve_with_expansion = AsyncMock(
                return_value=(
                    [
                        {
                            "id": "node_1",
            ...
            mock_retriever.format_for_context = MagicMock(
                return_value="## 知识图谱关联信息\n\n### Person\n- 实体1: 描述\n\n### 关系\n- 实体1 --[KNOWS]--> 实体2"
            )
            ...
            with patch.object(
                enhancer, "_search_nodes_by_keywords", new_callable=AsyncMock
            ) as mock_search:
                mock_search.return_value = {"node_1"}
                ...
                enhanced, graph_context = await enhancer.enhance(
                    memories, query="测试查询"
                )
    ...
    def test_extract_keywords(self, mock_component_manager):
        with patch(
            "iris_memory.enhancement.graph_enhancement.get_config"
        ) as mock_get_config:
            ...
            memories = [
                ...
                MemorySearchResult(
                    entry=MemoryEntry(
                        id="mem_2", content="记忆2", metadata={"kg_node_id": "node_1"}
                    ),
                    score=0.9,
                    distance=0.1,
                ),
            ...
            keywords = enhancer._extract_keywords(query="测试查询", memories=memories)

            assert isinstance(keywords, set)
            assert len(keywords) > 0

    def test_extract_keywords_deduplication(self, mock_component_manager):
        with patch(
            "iris_memory.enhancement.graph_enhancement.get_config"
        ) as mock_get_config:
            mock_get_config.return_value = MagicMock()
            ...
            memories = [
                MemorySearchResult(
                    entry=MemoryEntry(
                        id="mem_1", content="记忆1", metadata={"kg_node_id": "node_1"}
                    ),
                    score=0.9,
                    distance=0.1,
                ),
            ...
            keywords = enhancer._extract_keywords(query=None, memories=memories)

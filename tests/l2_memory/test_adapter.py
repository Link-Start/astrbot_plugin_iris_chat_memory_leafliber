...
class TestL2MemoryAdapter:
    ...
    async def test_initialize_success(self, mock_config, mock_chromadb):
        """测试初始化成功"""
        with (
            patch("iris_memory.l2_memory.adapter.get_config", return_value=mock_config),
    ...
    async def test_migrate_on_model_change_with_data(self):
        ...
        with (
            patch("iris_memory.l2_memory.adapter._ensure_chromadb"),
            patch("iris_memory.l2_memory.io.MemoryExporter") as mock_exporter_cls,
            patch("iris_memory.l2_memory.io.MemoryImporter") as mock_importer_cls,
        ):
            from iris_memory.l2_memory.io import ExportStats, ImportStats

            mock_exporter = Mock()
            mock_exporter.export_all = AsyncMock(
                return_value=ExportStats(
                    total_count=2,
                    exported_count=2,
                    skipped_count=0,
                    file_size_bytes=100,
            ...
            mock_importer = Mock()
            mock_importer.import_from_file = AsyncMock(
